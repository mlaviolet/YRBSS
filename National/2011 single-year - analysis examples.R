...
load("C:/Survey data/YRBSS/yrbs2011.rda")
# load( "C:/My Directory/YRBSS/yrbs2011.rda" )
...
y <- svydesign(id = ~psu, strata = ~stratum, data = x, 
...
svyby(~one, ~q2, y, unwtd.count)

# count the weighted number of individuals in yrbss #
svytotal(~one , y)
# note that this sums to the same total as the unweighted count
# most complex sample survey weights sum up to the total population
#   they represent; these do not.
# so if you want to count results in millions of americans:
# you'll have to divide the number of 9th-12th graders by the 
#   number of records in this data set

# basically, in the short-term, this just means that the `svytotal`
#   function won't be of much use to ya  ;)
...
# by sex
svyby(~bmipct, ~q2, design = y, svymean, na.rm = TRUE, na.rm.all = TRUE)
...
y <- update(qn11 = factor(qn11) , y)


# percent who had driven drunk in the past month
svymean(~qn11, design = y, na.rm = TRUE)

# by sex
svyby(~qn11, ~q2, design = y, svymean, na.rm = TRUE, na.rm.all = TRUE)

# calculate the median and other percentiles #

# minimum, 25th, 50th, 75th, maximum
# body mass index percentile within the respondent's age and sex cohort
svyquantile(~bmipct, design = y, c(0, 0.25, 0.5, 0.75, 1), na.rm = TRUE)

# by sex
svyby(~bmipct, ~q2, design = y, svyquantile, 	c( 0, 0.25, 0.5, 0.75, 1),
      ci = T,	na.rm = TRUE, na.rm.all = TRUE)
...
# average body mass index percentile within the respondent's age and sex cohort
svymean(~bmipct, design = y.female, na.rm = TRUE)
...
drunk.driving.by.sex <-
	svyby(~qn11, ~q2, design = y, svymean, na.rm = TRUE)
...
barplot(drunk.driving.by.sex[, 2],
        main = 
          "Percent of Adolescents Who Have Driven Drunk in the Past 30 Days",
